			...
			<div id="content">
				...
				<div class="__boardView __mt50">
					<div class="top">
						...
						<ul class="sumt">
									<? if( $table == "ddm_board_counsel" ){?>
							<li>
								<strong>분류</strong>
								<span><?=$row[board_kind]?></span>
							</li>
									<? } ?>
							<li>
								<strong>글쓴이</strong>
...
<?}?>
			...
			<?if( ($pwd == $row['board_pwd']&&$row['board_pwd']!='') || ($boardDeleteBtn && ( $row[board_id] == $_SESSION[member_id] || $_SESSION[masterSession] ))){?>&nbsp;<!-- 삭제 --><a><img src='../../images/btn/btn_delete.gif' alt="삭제" border="0" align="absmiddle" style="cursor:pointer" style="cursor:pointer;" onclick="if(confirm('삭제하시겠습니까?')) deleteForm.submit()"></a><?}?>

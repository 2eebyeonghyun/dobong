    ...
    <section class="section raise-page sub01-page">
        ...
        <article class="cont cont1">
            <div class="inner">
                <div id="content">
                    <div class="__tab4">
                        <a href="<?=DIR?>/study/care.php" class="active"><span>보육지원</span></a>
                        <a href="<?=DIR?>/study/raise.php"><span>양육지원</span></a>
                    </div>
                    ...
                    <div class="tac __mb30">
                        ...
                        <div style="float:right">
                            <form method="post" action="<?=$PHP_SELF?>">
                                ...
                                <? if(substr($pno,0,6) != "020601") { ?>
                                <select name="gubun" style="width:150px;" onChange="this.form.submit();">
                    ...
                    <div class="__scheTblWrap">
                        ...
                        //			$tmpEdu = " <A href= /html/sub/".$temp_index."?pno=".$pno."&mode=view&idx=".$mSCH['idx'][$i]."&toYear=".$mYear."&toMonth=".$mMonth."&sitem=".$sitem."&skey=".$skey."&page=".$mPage."><IMG src='".$tmpIcon."' onMouseMove='msgposit_list();' onMouseOver=\"msgset_list('".addslashes(htmlspecialchars($mSCH['title'][$i], NULL, 'ISO-8859-1'))."','".$mSCH['eTime'][$i]."');\" onMouseOut='msghide_list();'>";
                              ...
                              $tmpEdu = " <A href=care_view.php?pno=".$pno."&mode=view&idx=".$mSCH['idx'][$i]."&toYear=".$mYear."&toMonth=".$mMonth."&sitem=".$sitem."&skey=".$skey."&page=".$mPage." class='flt'>";
                              ...
                              if($mSCH['gubun'][$i]=="부모관련") $mEdu['3'] .= $tmpEdu."<span class='cir orange'>부</span>
                                                <div class='ov'><div class='in'><div class='suj'>".addslashes(htmlspecialchars($mSCH['title'][$i], NULL, 'ISO-8859-1'))."</div><p class='time'>".$mSCH['eTime'][$i]."</p>
                                                </div>
                                              </div>
                                            </a>";
                              ...
                              if($mSCH['gubun'][$i]=="영유아프로그램") $mEdu['5'] .= $tmpEdu."<span class='cir red'>영</span>
                                                <div class='ov'><div class='in'><div class='suj'>".addslashes(htmlspecialchars($mSCH['title'][$i], NULL, 'ISO-8859-1'))."</div><p class='time'>".$mSCH['eTime'][$i]."</p>
                                                </div>
                                              </div>
                                            </a>";
                    ...
                    <table class="__tblList respond2">
                        ...
                        <tbody>
                              ...
                              if($_SESSION['masterSession']){
                                ...
                                while($rs = mysql_fetch_array($result)){
                                  ...
                                  if($winwon > 0){	// 대기인원 설정이 있는 경우
                                    ...
                                    if($appInwon >= $inwon && $appInwon < ($inwon+$winwon)){
                                      $status = "<span style='color:blue'>대기</span>";
                                    } elseif($appInwon >= ($inwon+$winwon)){
                            ...
                            ?>
                            <tr>
							                  <td class="__p"><?=$mEduDate?></td>
							                  <td><?=date("Y-n-d H:00",$rs[req_sdate])."<br /> ~ ".date("Y-n-d H:00",$rs[req_edate])?></td>
							                  <td class="subject"><A href="care_view.php?pno=<?=$pno?>&mode=view&idx=<?=$rs[idx]?>&toYear=<?=$mYear?>&toMonth=<?=$mMonth?>&sitem=<?=$sitem?>&skey=<?=$skey?>&                 page=<?=$mPage?>"><?=$rs[title]?></A></td>
...
<script language="javascript">
    ...
    function msgset_list(v1, v2) {
        ...
        text += "			<table width='166' border='0' align='center' cellpadding='0' cellspacing='0" +
                "'>";
        text += "				<tr>";
        text += "					<td width='9' style='vertical-align:top;padding:3 0 0 0;'><img src='../.." +
                "/images/common/dot_02.gif' ></td>";
        text += "					<td width='157' class='text_01'>" + v1 + "</td>";
        text += "				</tr>";
        text += "				<tr>";
        text += "					<td>&nbsp;</td>";
        text += "					<td valign='top' class='text_02' style='padding:3 0 0 0;'>" + v2 +
                "</td>";
        text += "				</tr>";
        text += "			</table>";
        text += "		</td>";

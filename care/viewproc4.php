	...
	$query	= "SELECT count(*) FROM ddm_daynursery_app WHERE parent_idx in(select idx from ddm_daynursery where left(FROM_UNIXTIME(edu_sdate), 10) = (select left(FROM_UNIXTIME(edu_sdate), 10) from ddm_daynursery where idx = '$parent_idx')) && mbId='$mbId' and del_yn='N'";


		//echo "<script>alert(\"$query\");</script>";
	$appchk = sqlRowOne($query);
	if($appchk > 0){
		echo "<script>alert('예약 신청은 1일 1회만 가능합니다. 해당일은 이미 신청되었습니다.');location.href = 'association_schedule4.php?pno=020207&mode=list4&scd=01';</script>";
		exit;
	}
...
//	}
	...
	if(!$winwon){ // 대기신청이 없을때
		if($temp_inwon > $inwon_empty){
			echo "<script>alert('신청인원이 초과되었습니다. 신청가능인원은 ".$inwon_empty.$self_count."명 입니다.');</script>";
			exit;
		}else{
			$status1 = 1; //신청
		}

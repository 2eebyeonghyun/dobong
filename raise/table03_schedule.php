...
<?
$_dep = array(3,7,1);
$_tit = array('양육지원','상차림대여사업','휘경점 신청하기');
...
<? if(true){ ?>
		if(confirm('예약신청 하시겠습니까?')){
			location.href='03010309_proc.php?pno=03010309&mode=proc&barcode='+barcode+'&sdate='+sdate+'&type=app';
			return true;
		}
<? } else{ ?>
...
<div class="__tab3">
					...
					<a href="<?=DIR?>/raise/table03.html" class="active"><span>휘경점 상차림  신청</span></a>
				</div>
...
if($dayClass == 'normal' && trim($mSCH['day']) != "" && trim($row['ho_name'])=="") {
						$itemInfo = array(array('전통상차림', 'DM03TG00000201'), array('모던상차림', 'DM03TG00000301'));

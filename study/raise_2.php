...
부모,영유아를 위한 교육 및 행사를 신청하실 수 있습니다.
				...
				<div class="__tab __mt10">
					<a href="raise.php"><span>답십리점</span></a>
...
//			$tmpEdu = " <A href= /html/sub/".$temp_index."?pno=".$pno."&mode=view&idx=".$mSCH['idx'][$i]."&toYear=".$mYear."&toMonth=".$mMonth."&sitem=".$sitem."&skey=".$skey."&page=".$mPage."><IMG src='".$tmpIcon."' onMouseMove='msgposit_list();' onMouseOver=\"msgset_list('".addslashes(htmlspecialchars($mSCH['title'][$i], NULL, 'ISO-8859-1'))."','".$mSCH['eTime'][$i]."');\" onMouseOut='msghide_list();'>";
			...
			$tmpEdu = " <A href=raise2_view.php?pno=".$pno."&mode=view&idx=".$mSCH['idx'][$i]."&toYear=".$mYear."&toMonth=".$mMonth."&sitem=".$sitem."&skey=".$skey."&page=".$mPage." class='flt'>";
...
?>
						<tr>
							...
							<td class="subject"><A href="raise2_view.php?pno=<?=$pno?>&mode=view&idx=<?=$rs[idx]?>&toYear=<?=$mYear?>&toMonth=<?=$mMonth?>&sitem=<?=$sitem?>&skey=<?=$skey?>&page=<?=$mPage?>"><?=$rs[title]?></A></td>
...
<!--
						<tr>
							...
							<td class="subject"><a href="./raise2_view.html">[배봉산점] 꿈자람 공동육아방 - 1회차(10:00~11:30)</a></td>
							<td>320 / 120</td>
							<td class="__p"></td>
						</tr>
-->

...
if($mode=="setExt"){
	...
	echo "<script>location.href='../mypage/toy.php';</script>";
	exit;
}
...
//	} else {
	...
	// 답십리점 - 목요일은 오후 5시30분까지 예약 가능

/*fix.코로나로 인해 목요일 연장운영 잠정 중단 20210709
	if($s_cd=='01' && date('w')==4){
		if( date("H.i",$sdate)<09.30 || date("H.i",$sdate) > 17.30){
			echo "<script>alert('매주 목요일 대여예약은 오전 9시30분부터 오후 5시30분까지 가능합니다.');history.back();</script>";
			exit;
		}
	} else{
*/
		if( date("H.i",$sdate)<09.30 || date("H.i",$sdate) > 15.30){
			echo "<script>alert('대여예약은 오전 9시30분부터 오후 3시30분까지만 가능합니다.');history.back();</script>";
			exit;
		}
/*fix.코로나로 인해 목요일 연장운영 잠정 중단 20210709
	}
...
//	$rentNumChk1 = sqlRowOne("select count(*) from ona_item_reserve a left join ona_item_barcode b on a.barcode=b.barcode where a.status in ('대여완료','예약완료') and b.s_cd='01' and a.regdate like '".$today."%'");
	...
	if($rentNumChk1>4 && $s_cd=='01'){
		echo "<script>alert('장난감대여 동시간대 (5명)예약이 마감되었습니다. 추후 다시 이용해 주시기 바랍니다.\\n자세한 사항은 센터로 문의해 주시기 바랍니다. Tel. 02)2237-5800');location.href='../raise/dab3.php';</script>";
		exit;
	}
...
if($_SERVER['REMOTE_ADDR']=="1.209.234.24"){
	echo "select count(*) from ona_item_reserve a left join ona_item b on a.itemcode=b.itemcode where a.userid='$userid' and a.memberno='$memberno' and a.edate>'$sdate' and a.status='예약완료' and b.s_cd='$s_cd' and b.itemtype!='C'";
//	exit;
}

	if($tmpChk){
//		echo "<script>if(confirm('해당 카드번호는 이미 예약중인 상품이 있습니다.\\n마이페이지에서 예약중인 내역을 취소하시면 새로 예약을 하실수 있습니다.\\n예약내역을 확인하러 가시겠습니까?')) location.href='../mypage/reserve.php';</script>";
		echo "<script>if(confirm('해당 카드번호는 이미 예약중인 상품이 있습니다.\\n예약내역을 확인하러 가시겠습니까?')) location.href='../mypage/reserve.php';</script>";
		exit;
	}
...
/*fix.
	$tmpChk2 = sqlRowOne("select count(*) as cnt from ona_orderitem a left join ona_item b on a.itemcode=b.itemcode where a.userid='$userid' and a.memberno='$memberno' and a.status = 'RT03' and b.s_cd='$s_cd' and b.itemtype!='C'and delete_yn = 'N'");

	if($tmpChk2 > 5) {
		echo "<script>if(confirm('해당 카드번호는 이미 대여중인 상품이 있습니다.\\n대여내역을 확인하러 가시겠습니까?')) location.href='../mypage/toy.php';</script>";
		exit;
	}
*/
	...
	if($barcodeinfo['status']!="LD01"){
		echo "<script>alert('예약이 불가능한 상태의 물품입니다.');location.href='../raise/dab3.php';</script>";
		exit;
	}
	...
	if($tmpChk > 1){
		...
		if($reserveinfo_tmp[memberno] != $memberno){
			...
			echo "<script>alert('이미 예약이 되어있습니다. 자동 새로고침 됩니다.');location.href='../mypage/reserve.php';</script>";
			exit;
		}
	}

	echo "<script>alert('예약 되었습니다.');location.href='../mypage/reserve.php';</script>";
	exit;
}
...
if($mode=="cancelReserve"){
	...
	if($reserveinfo['status']!="예약완료"){
		echo "<script>alert('취소가 불가능한 상태입니다.');location.href='../mypage/reserve.php';</script>";
		exit;
	}
	...
	echo "<script>alert('예약취소 되었습니다.');location.href='../mypage/reserve.php';</script>";
	exit;
}
?>
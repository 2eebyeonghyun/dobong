...
//답십리
   ...
   if($type == 'mod')
   {
	   $listQuery		= "SELECT * FROM ddm_usecenter_app2 WHERE idx='".$idx."' ORDER BY idx DESC";
	   ...
	   $time_check = sqlArrayone("select time from ddm_usecenter_app2 where wdate='".$mod_row['wdate']."' and userid != '".$mod_row['userid']."'" );
...
<?
    ...
    $usecount = sqlArrayOne("select count(*) from ddm_usecenter2 where left(wdate,7) = '$cyear-$cmonth'");
...
<div class="__tab3">
					<a href="<?=DIR?>/care/borrow.php"><span>답십리점 대관 신청하기</span></a>
					<a href="<?=DIR?>/care/borrow.php" class="active"><span>제기점 대관 신청하기</span></a>
				</div>
...
<?
    function DispCell($mCellIdx,$mYear,$mMonth,$mSCH){
		...
		$use_count = sqlArrayOne("select count(*) from ddm_usecenter2 where left(wdate,10) = '".$wdate."' and usetime='1'");
		$use_count2 = sqlArrayOne("select count(*) from ddm_usecenter2 where left(wdate,10) = '".$wdate."' and usetime='2'");
		$reg_count = sqlArrayOne("select count(*) from ddm_usecenter_app2 where left(wdate,10) = '".$wdate."' and time='1'");
		$reg_count2 = sqlArrayOne("select count(*) from ddm_usecenter_app2 where left(wdate,10) = '".$wdate."' and time='2'");
		$my_count = sqlArrayOne("select count(*) from ddm_usecenter_app2 where left(wdate,10) = '".$wdate."' and time='1' and userid = '".$_SESSION[member_id]."'");
        $my_count2 = sqlArrayOne("select count(*) from ddm_usecenter_app2 where left(wdate,10) = '".$wdate."' and time='2' and userid = '".$_SESSION[member_id]."'");
...
<div class="__popBasic popApp">
	...
	<form name="frm" method="post" action="020207_proc2.php" onSubmit="return appCheck(this);" >
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<label class="checkbtn"><input type="checkbox" name="agree04" id="agree04"> 전체 인원 준수(방문자, 참석자, 진행자 포함 동시간대 최대 25명)</label><br><br>
